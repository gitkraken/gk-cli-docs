...
<!--->
image of finished code suggest
--->

To start, navigate (`cd`) to a repository with an open pull request. Then, check out the branch with the open pull request (`git checkout branch-name`). Next, begin making the desired changes you would like to include as suggestions. You can open the repository in GitKraken Client or GitLens for VS Code using `gk graph`.
